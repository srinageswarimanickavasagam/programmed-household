...
import srinageswari.programmedhousehold.service.item.IItemService;
...
public class ItemController {
   ...
   */
  @GetMapping("/item/{id}")
  public ResponseEntity<APIResponseDTO<ItemResponseDTO>> findById(@PathVariable long id) {
    final ItemResponseDTO response = itemService.findById(id);
   ...
   */
  @GetMapping("/items")
  public ResponseEntity<APIResponseDTO<Page<ItemResponseDTO>>> findAll(
      @RequestBody SearchRequestDTO request) {
    final Page<ItemResponseDTO> response = itemService.findAll(request);
    return ResponseEntity.ok(
        new APIResponseDTO<>(Instant.now().toEpochMilli(), Constants.SUCCESS, response));
  }
