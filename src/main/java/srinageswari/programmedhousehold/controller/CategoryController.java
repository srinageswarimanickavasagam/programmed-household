...
public class CategoryController {
  ...
  public ResponseEntity<APIResponseDTO<CategoryResponseDTO>> findById(@PathVariable long id) {
    final CategoryResponseDTO response = categoryService.findById(id);
  ...
  public ResponseEntity<APIResponseDTO<Page<CategoryResponseDTO>>> findAll(
      @RequestBody SearchRequestDTO request) {
    final Page<CategoryResponseDTO> response = categoryService.findAll(request);
    return ResponseEntity.ok(
        new APIResponseDTO<>(Instant.now().toEpochMilli(), Constants.SUCCESS, response));
  }
