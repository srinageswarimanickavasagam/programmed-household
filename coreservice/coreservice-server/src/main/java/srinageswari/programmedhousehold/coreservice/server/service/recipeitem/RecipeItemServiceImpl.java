...
import static srinageswari.programmedhousehold.coreservice.server.common.Constants.ALREADY_EXISTS_ITEM;
import static srinageswari.programmedhousehold.coreservice.server.common.Constants.NOT_FOUND_ITEM;
...
import srinageswari.programmedhousehold.coreservice.server.repository.RecipeItemRepository;
    ...
    if (request.getItem().getId() != 0) {
      ...
      if (recipeItemRepository.existsByRecipeIdAndItemId(
        ...
        throw new ElementAlreadyExistsException(
            String.format(ALREADY_EXISTS_ITEM, request.getItem().getId()));
      }
      ...
      if (itemRepository.existsByNameIgnoreCase(request.getItem().getName())) {
        log.error(ALREADY_EXISTS_ITEM);
        throw new ElementAlreadyExistsException(
            String.format(ALREADY_EXISTS_ITEM, request.getItem()));
      }
